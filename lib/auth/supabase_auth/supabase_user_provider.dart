...
class PPADSGrupo4SupabaseUser extends BaseAuthUser {
  PPADSGrupo4SupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> pPADSGrupo4SupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PPADSGrupo4SupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
